...
if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.19.3 - authlib 3.16.29"
  curl -O --silent "$1"common/1.19.3/authlib-3.16.29.jar -u "$2:$3"
  echo "Download spigot authlib 3.16.29 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.19.3 - spigot 1.19.3"
  curl -O --silent "$1"common/1.19.3/spigot-1.19.3-R0.1-SNAPSHOT.jar -u "$2:$3"
  echo "Download spigot 1.19.3 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.19.3 - datafixerupper 5.0.28"
...
if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.19.4 - authlib 3.17.30"
  curl -O --silent "$1"common/1.19.4/authlib-3.17.30.jar -u "$2:$3"
  echo "Download spigot authlib 3.17.30 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.19.4 - spigot 1.19.4"
  curl -O --silent "$1"common/1.19.4/spigot-1.19.4-R0.1-SNAPSHOT.jar -u "$2:$3"
  echo "Download spigot 1.19.4 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.19.4 - datafixerupper 6.0.6"
...
if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.20.1 - authlib 4.0.43"
  curl -O --silent "$1"common/1.20.1/authlib-4.0.43.jar -u "$2:$3"
  echo "Download spigot authlib 4.0.43 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.20.1 - spigot 1.20.1"
  curl -O --silent "$1"common/1.20.1/spigot-1.20.1-R0.1-SNAPSHOT.jar -u "$2:$3"
  echo "Download spigot 1.20.1 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.20.1 - datafixerupper 6.0.8"
...
if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.20.2 - authlib 5.0.47"
  curl -O --silent "$1"common/1.20.2/authlib-5.0.47.jar -u "$2:$3"
  echo "Download spigot authlib 5.0.47 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.20.2 - spigot 1.20.2"
  curl -O --silent "$1"common/1.20.2/spigot-1.20.2-R0.1-SNAPSHOT.jar -u "$2:$3"
  echo "Download spigot 1.20.2 - OK"
fi

if [ -n "$1" ]; then
  echo "info : using private cache ci url for 1.20.2 - datafixerupper 6.0.8"
